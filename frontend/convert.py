...
def verify_duckdb_database(db_path: str):
    ...
    try:
        ...
        queries = [
            ...
            ("Top 5 regions", "SELECT region, COUNT(*) as count FROM athletes WHERE region IS NOT NULL GROUP BY region ORDER BY count DESC LIMIT 5"),
            ("Sample athlete data", "SELECT name, gender, age, weight, deadlift FROM athletes WHERE name IS NOT NULL LIMIT 3")
        ]
