    ...
    """
        ...
        with st.sidebar:
            ...
            with st.sidebar.container():
                st.subheader(
                    f"{x_axis_display} ({helpers.get_event_info(x_axis, 'unit')})"
                )
            ...
            with st.sidebar.container():
                st.subheader(
                    f"{y_axis_display} ({helpers.get_event_info(y_axis, 'unit')})"
                )
            ...
            sample_size = st.radio(
                "Select a sample size", options=[1000, 10000, 100000]
            )
            (
                df,
                (x_mean, x_std),
        ...
        with scatter_tab:
            fig = generate_scatter_plot(
                df=df, x_axis=x_axis, y_axis=y_axis, trendline=trendline
            )
            ...
            if helpers.get_event_info(x_axis, "better") == "lower":
                top_athletes = df.nsmallest(5, x_axis)[
                    ["name", x_axis, y_axis]
                ].to_dict("records")
            else:
                top_athletes = df.nlargest(5, x_axis)[["name", x_axis, y_axis]].to_dict(
                    "records"
                )
